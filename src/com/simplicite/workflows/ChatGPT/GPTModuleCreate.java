...
public class GPTModuleCreate extends Processus {
	...
	private static final String ACTIVITY_GEN ="GPTC_0600";
	...
	public String chatBot(Processus p, ActivityFile context, ObjectContextWeb ctx, Grant g){
		...
		String contextApp =GptTools.gptCaller(g, "you help to describe UML for non technical person","Describes the application defined by this JSON in a graphical way for non-technical users: "+json.toString() , null,false,true).getJSONArray("choices").getJSONObject(0).getJSONObject("message").getString("content");
		contextApp = formatAnswerGpt(contextApp);
	...
	public String gpt(Processus p, ActivityFile context, ObjectContextWeb ctx, Grant g){
		...
		if(listResult.size()!=3)return Message.formatError("GPT_ERROR_RETURN", listResult.get(0),null );
	...
	private String getPromptFromPromptActivity(Grant g){
		String prompt = getContext(getActivity(ACTIVITY_PROMPT)).getDataValue("Data", "gpt_prompt");
		if(Tool.isEmpty(prompt)){//for test
			return "";
		}
	...
	public String gen(Processus p, ActivityFile context, ObjectContextWeb ctx, Grant g){
		...
		try{
			JSONObject jsonObject = GptTools.getValidJson(json);
			if(Tool.isEmpty(jsonObject)){
				return g.getText("GPT_JSON_ERROR");
			}
			List<String> ids = GptModel.genModule(moduleId,	groupIds,domainId,jsonObject);
			...
			if(Tool.isEmpty(objs)){
				return "<p>"+g.getText("GPT_SUCCESS")+"</p><script>" + g.getExternalObject(PROCESS_RESOURCE_EXTERNAL_OBJECT).getResourceJSContent("GPT_GEN_MODEL")+"\n"+ "gptNewModel();"+"</script>";
			}else{
				ids.addAll(Arrays.asList(objs));
				ids = new ArrayList<>(new HashSet<>(ids));
				context.setDataFile("Data", "allIds", ids);
				return "<p>"+g.getText("GPT_COMPLETED")+"</p><script>" + g.getExternalObject(PROCESS_RESOURCE_EXTERNAL_OBJECT).getResourceJSContent("GPT_GEN_MODEL")+"\n"+ "gptNewModel();"+"</script>";
			}
	...
	private String formatAnswerGpt(String answer){
